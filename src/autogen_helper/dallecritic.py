...
class AzureDalleImageGenerator:
    ...
    def __init__(
        ...
        """
        Args:
            llm_config: LLM config, must contain a valid DALL-E model and Azure OpenAI API key in config_list.
            resolution: The resolution of the image you want to generate. Must be one of "256x256", "512x512", "1024x1024", "1792x1024", "1024x1792".
            quality: The quality of the image you want to generate. Must be one of "standard", "hd".
        ...
        self.az_manager = AzureOpenAIManager(
...
class ImageGeneration(AgentCapability):
    ...
    def __init__(
        ...
        """
        Args:
            image_generator: The image generator you would like to use to generate images.
            cache: The cache client to use to store and retrieve generated images. If None, no caching will be used.
            text_analyzer_llm_config: The LLM config for the text analyzer. If None, the LLM config will be retrieved from the agent you're adding the ability to.
            text_analyzer_instructions: Instructions provided to the TextAnalyzerAgent used to analyze incoming messages and extract the prompt for image generation.
            verbosity: The verbosity level. Defaults to 0 and must be greater than or equal to 0. The text analyzer LLM calls will be silent if verbosity is less than 2.
            register_reply_position: The position of the reply function in the agent's list of reply functions. Defaults to 2 to place it after the check termination and human reply for a ConversableAgent.
        """
    ...
    def add_to_agent(self, agent: ConversableAgent):
        ...
        3. Updates System Message: The agent's system message is updated to include a message indicating the capability
           to generate images has been added.
        ...
        Args:
            agent: The ConversableAgent to add the capability to.
        """
        self._agent = agent
    ...
    def _extract_prompt(self, last_message: str) -> str:
        assert self._text_analyzer is not None
    ...
    def _extract_analysis(self, analysis: Union[str, Dict[str, Any], None]) -> str:
...
def _validate_dalle_model(model: str):
    """Checks if the provided DALL-E model is valid.
    
    Args:
        model: The name of the DALL-E model.
